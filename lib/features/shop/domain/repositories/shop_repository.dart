...
class ShopRepository implements ShopRepositoryInterface{
  ...
  Future<http.StreamedResponse> updateShop(
      ...
      XFile? offerBanner,
      {
        ...
        List<String>? selectedGovernoratesIds, // <-- أضف هذا المتغير
      }
      ) async {
    http.MultipartRequest request = http.MultipartRequest(
      'POST',
      Uri.parse('${AppConstants.baseUrl}${AppConstants.shopUpdate}'),
    );
    ...
    if (shopBanner != null) {
      ...
      var part = http.MultipartFile(
        'banner',
        shopBanner.readAsBytes().asStream(),
        list.length,
    ...
    if (secondaryBanner != null) {
      ...
      var part = http.MultipartFile(
        'bottom_banner',
        secondaryBanner.readAsBytes().asStream(),
        list.length,
    ...
    if (offerBanner != null) {
      ...
      var part = http.MultipartFile(
        'offer_banner',
        offerBanner.readAsBytes().asStream(),
        list.length,
    ...
    // أضف المحافظات بنفس الشكل المطلوب
    if (selectedGovernoratesIds != null) {
      for (int i = 0; i < selectedGovernoratesIds.length; i++) {
        fields['governorate_ids[$i]'] = selectedGovernoratesIds[i];
      }
    }
