...
class ProfileInfoModel {
  ...
  // ✅ جديد
  List<Governorate>? sellerGovernorates;
  int? points;
  ...
  ProfileInfoModel.fromJson(Map<String, dynamic> json) {
    ...
    try {
      minimumOrderAmount = json['minimum_order_amount']?.toDouble() ?? 0;
    } catch (e) {
      minimumOrderAmount = double.parse(json['minimum_order_amount'].toString());
    }

    try {
      freeOverDeliveryAmount = json['free_delivery_over_amount']?.toDouble() ?? 0;
    } catch (e) {
      freeOverDeliveryAmount = double.parse(json['free_delivery_over_amount'].toString());
    }

    try {
      freeOverDeliveryAmountStatus = json['free_delivery_status'] ?? 0;
    } catch (e) {
    ...
    if (json['governorates'] != null) {
      sellerGovernorates = List<Governorate>.from(
        json['governorates'].map((e) => Governorate.fromJson(e)),
      );
    }
...
class Governorate {
  ...
  Governorate({this.id, this.name, this.note, this.minShippingCost});
  ...
  factory Governorate.fromJson(Map<String, dynamic> json) {
    return Governorate(
      ...
      name: json['name'],
      note: json['note'],
      minShippingCost: double.tryParse(json['min_shipping_cost'].toString()),
    );
  }
}
