...
class ProfileScreenState extends State<ProfileScreen> {
  ...
  _updateUserAccount() async {
    ...
    && _passwordController.text.isEmpty && _confirmPasswordController.text.isEmpty) {
      ...
      Provider.of<ShopController>(context, listen: false).selectedGovernorateIds=Provider.of<ProfileController>(context, listen: false).selectedGovernorateIds;
     await Provider.of<ShopController>(context, listen: false).updateShopInfo();
    }
  }
  ...
  void initState() {
    super.initState();
    Provider.of<ProfileController>(context, listen: false).getGovernates();
          ...
          return Consumer<ProfileController>(
            builder: (context, profile, child) {
              if(_firstNameController.text.isEmpty || _lastNameController.text.isEmpty) {
                ...
                String countryCode = CountryCodeHelper.getCountryCode(profile.userInfoModel?.phone??"0")??"0";
                      ...
                      child: Column(
                        children: [
                          ...
                          MultiSelectDialogField(
                            items: profile.governates?.map((g) => MultiSelectItem<String>(g.name ?? '', g.name ?? ''))
                                .toList()??[],
                            title: Text(getTranslated('select_governorate', context) ?? ""),
                            ...
                            buttonText: Text(
                              getTranslated('select_governorate', context) ?? "",
                              style: const TextStyle(
                                color: Colors.blue,
                                fontSize: 16,
                              ),
                            ),
                            initialValue: profile.selectedGovernorateNames ?? [],
                            onConfirm: (selected) {
                              profile.selectedGovernorateNames = List<String>.from(selected);

                              profile.selectedGovernorateIds = profile.governates?.where((g) => profile.selectedGovernorateNames!.contains(g.name))
                                  .map((g) => g.id ?? 0)
                                  .toList()??[];

                              print("✅ Selected names: ${profile.selectedGovernorateNames}");
                              print("✅ Selected ids: ${profile.selectedGovernorateIds}");
                            },
